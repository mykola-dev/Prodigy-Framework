...
inline fun <reified T : Activity> Context.runActivity(b: Bundle? = null) {
...
inline fun <reified T : Activity> Activity.runActivity(requestCode: Int, b: Bundle? = null) {
    val i = Intent(this, T::class.java)
    if (b != null)
        i.putExtras(b)
...
inline fun <reified T : Activity> Context.runActivity(noinline f: BundleBuilder.() -> Unit) {
    val b = bundle(f)
    runActivity<T>(b)
}

inline fun <reified T : Activity> Activity.runActivity(requestCode: Int, noinline f: BundleBuilder.() -> Unit) {
    val b = bundle(f)
    runActivity<T>(requestCode, b)
}
